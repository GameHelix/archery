...
export default function HashToolsPage() {
  ...
  const getAlgorithmColor = (algorithm: HashAlgorithm) => {
    switch (algorithm) {
      case 'MD5': return 'bg-red-500/20 text-red-400'
      case 'SHA-1': return 'bg-yellow-500/20 text-yellow-400'
      case 'SHA-256': return 'bg-green-500/20 text-green-400'
      case 'SHA-512': return 'bg-blue-500/20 text-blue-400'
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="bg-dark-card backdrop-blur-sm rounded-2xl shadow-xl border border-dark-700 hover:border-primary-500/50 transition-all duration-300 p-6 sm:p-8">
              <div className="flex items-center mb-6">
                <Hash className="h-6 w-6 text-slate-400 mr-3" />
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border-2 transition-all duration-200 ${
                        selectedAlgorithms.includes(algorithm as HashAlgorithm)
                          ...
                          : 'border-dark-700 bg-dark-800 hover:border-slate-300 hover:bg-slate-500/10 text-gray-300'
                      }`}
                    >
                      <div className="font-medium text-sm">{algorithm}</div>
                      <div className="text-xs opacity-75">{info.bits} bit</div>
                      {info.deprecated && (
                        <div className="text-xs text-red-400 mt-1">⚠️ Deprecated</div>
                      )}
                    </button>
                  ))}
                </div>
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            <div className="bg-dark-card backdrop-blur-sm rounded-2xl shadow-xl border border-dark-700 hover:border-primary-500/50 transition-all duration-300 p-6">
              <div className="flex items-center mb-4">
                <Hash className="h-6 w-6 text-green-400 mr-2" />
            ...
            <div className="bg-gradient-to-br from-amber-500/20 to-orange-500/20 rounded-2xl p-6 border border-amber-500/30">
              <h3 className="text-lg font-semibold text-amber-300 mb-4">Security Notice</h3>
